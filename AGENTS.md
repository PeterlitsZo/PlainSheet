...
- Native Typst rendering is implemented in `native/src/lib.rs` and loaded via `src/main/native.ts`.
...
- The preview pipeline is intentionally "main-process cache + custom protocol URL". Do not revert to sending full PNG payloads over IPC to the renderer.
...
- For preview-flow changes, do a local manual check by editing Typst content and confirming continuous preview updates.
